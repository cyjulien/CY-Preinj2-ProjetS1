...
double calculateLeak(Instance* node, double incomingVolume) {
    ...
    double leakSum = 0.0;
    double childVolume = incomingVolume / node->downstreamCount;

    for (int i = 0; i < node->downstreamCount; i++) {
        Instance* child = node->downstream[i];
        double childLeak = childVolume * (child->leaks / 100.0); // leaks %
        leakSum += childLeak;

        // Remaining volume goes to downstream nodes
        leakSum += calculateLeak(child, childVolume - childLeak);
    }
